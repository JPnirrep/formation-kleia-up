  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6" role="region" aria-label="Formulaire formation">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/admin/courses')} className="text-sm text-kleia-gray hover:text-kleia-burgundy transition-colors" aria-label="Retour à la liste">← Retour</button>
        ...
        <Card>
          <div className="space-y-4">
            <div>
              ...
              <input id="title" type="text" required value={form.title} onChange={(e) => handleChange('title', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body" placeholder="Ex: L'Architecture du Message" />
            </div>
            <div>
              ...
              <input id="short_description" type="text" value={form.short_description} onChange={(e) => handleChange('short_description', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body" placeholder="Brève description (carte)" />
            </div>
            <div>
              <label htmlFor="description" className="block text-sm font-medium text-kleia-dark font-body mb-1">Description complète</label>
              <textarea id="description" rows={4} value={form.description} onChange={(e) => handleChange('description', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body resize-y" placeholder="Description détaillée de la formation..." />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="level" className="block text-sm font-medium text-kleia-dark font-body mb-1">Niveau</label>
                <select id="level" value={form.level} onChange={(e) => handleChange('level', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body bg-white">
                  {LEVELS.map((l) => <option key={l} value={l}>{l}</option>)}
                </select>
              </div>
              <div>
                <label htmlFor="category" className="block text-sm font-medium text-kleia-dark font-body mb-1">Catégorie</label>
                <select id="category" value={form.category} onChange={(e) => handleChange('category', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body bg-white">
                  <option value="">—</option>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input id="duration" type="number" min={0} value={form.duration_seconds} onChange={(e) => handleChange('duration_seconds', parseInt(e.target.value) || 0)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body" />
              </div>
              <div>
                <label htmlFor="status" className="block text-sm font-medium text-kleia-dark font-body mb-1">Statut</label>
                <select id="status" value={form.status} onChange={(e) => handleChange('status', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-kleia-dark/20 focus:border-kleia-burgundy focus:ring-2 focus:ring-kleia-burgundy/20 outline-none transition-all font-body bg-white">
...
with open(
    "C:\\Users\\JP\\Documents\\GitHub\\formation-kleia-up\\frontend\\src\\pages\\AdminCourseForm.tsx",
    "w",
    encoding="utf-8",
) as f:
    f.write(content)
