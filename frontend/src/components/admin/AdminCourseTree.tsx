            ...
            return (
              <div key={mod.id}>
                <div
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onToggleModule(mod.id); } }}
                >
                  ...
                  <button onClick={(e) => { e.stopPropagation(); if (confirm('Supprimer ce module et ses leçons ?')) onDeleteModule(mod.id); }} className="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity ml-1 text-xs" aria-label={`Supprimer le module ${mod.title}`}>✕</button>
                </div>
