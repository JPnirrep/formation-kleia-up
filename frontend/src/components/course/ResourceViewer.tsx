  ...
  return (
    <div className="mt-8 space-y-6">
      <h3 className="text-xl font-bold font-heading text-kleia-dark flex items-center gap-2">
        <svg className="w-6 h-6 text-kleia-violet" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {resources.map((resource) => (
          <motion.div
            key={resource.id}
            whileHover={{ y: -2 }}
            className="glass-dark p-4 flex items-center justify-between group cursor-pointer border-transparent hover:border-kleia-violet/20 transition-all"
          >
            ...
            <a 
              ...
              className="p-2 bg-kleia-violet text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
            >
