...
export default function PlayerShell({ video, className, onTimeUpdate }: PlayerShellProps) {
  ...
  const sendProgressRef = useRef((_completed?: boolean, _currentTime?: number, _duration?: number) => {});
  sendProgressRef.current = (completed = false, ct?: number, dur?: number) => {
    const v = videoRefState.current;
    if (!v) return;

    const el = videoRef.current;
    const currentTimeVal = ct ?? (el ? el.currentTime : 0);
    const durationVal = dur ?? (el ? el.duration : 0);

    if (currentTimeVal > maxPositionRef.current) {
      maxPositionRef.current = currentTimeVal;
    }

    const percentWatched = durationVal > 0 ? Math.round((currentTimeVal / durationVal) * 100) : 0;
    const isCompleted = completed || percentWatched >= 95;

    updateVideoProgress(v.id, {
      last_position_seconds: currentTimeVal,
      max_position_seconds: maxPositionRef.current,
  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      ...
      if (el.currentTime > maxPositionRef.current) {
        maxPositionRef.current = el.currentTime;
      }
    };
      ...
      <div className={clsx('relative w-full aspect-video bg-black overflow-hidden rounded-kleia', className)}>
        <YouTubePlayer 
          ...
          onProgress={(_percent, currentTime, duration) => {
            ...
            sendProgressRef.current(false, currentTime, duration);
          }}
          onEnded={() => {
