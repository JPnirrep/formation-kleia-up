...
export default function AdminUsers() {
  useEffect(() => { document.title = 'Utilisateurs — Kleia-up'; }, []);
  ...
  return (
    <div className="space-y-6" role="region" aria-label="Gestion des utilisateurs">
      <div className="flex items-center justify-between">
        ...
        <Button variant="primary" size="sm" data-testid="admin-users-create" onClick={() => setShowCreate(true)}>
      ...
      {showCreate && (
        <Card className="border-l-4 border-kleia-gold">
          <h2 className="font-heading font-bold text-kleia-dark mb-3">Nouvel utilisateur</h2>
          <div className="flex flex-col gap-3">
            <input
              ...
              className="px-3 py-2 rounded-lg border border-kleia-dark/20 text-sm font-body outline-none focus:border-kleia-violet focus:ring-2 focus:ring-kleia-gold focus:ring-offset-1"
              autoFocus
            />
            <input
              type="email"
                ...
                <tr key={user.id} className="border-b border-kleia-dark/5 last:border-0 hover:bg-kleia-dark/5 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <Button
                      ...
                      aria-label={`${user.is_active ? 'Désactiver' : 'Réactiver'} ${user.display_name}`}
                    >
