...
export default function LoginPage() {
  ...
  useEffect(() => { document.title = 'Connexion — Kleia-up'; }, []);
      ...
      <div className="w-full max-w-md">
        <div className="bg-white/70 backdrop-blur-[16px] border border-white/20 shadow-glass rounded-2xl p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-extrabold font-heading text-kleia-violet tracking-tight">Bienvenue</h1>
            <p className="mt-2 text-sm text-kleia-gray font-body">Connectez-vous pour accéder à vos formations</p>
          </div>

          {/* ── Déjà membre ── */}
          <p className="text-xs font-semibold text-kleia-violet uppercase tracking-wider mb-4">Déjà membre</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && <div data-testid="login-error" className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">{error}</div>}
            <button type="submit" disabled={isBusy}
              data-testid="login-submit"
              className="w-full py-2.5 px-4 rounded-lg gradient-violet text-white font-semibold text-sm font-heading hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-kleia-gold focus-visible:ring-offset-2 focus-visible:outline-none">
              {isBusy ? 'Connexion…' : 'Se connecter'}
            </button>
            <p className="text-center text-sm text-kleia-gray">
              <Link to="/forgot-password" data-testid="login-forgot-password" className="text-kleia-violet font-medium hover:underline">Mot de passe oublié ?</Link>
            </p>
          </form>
          ...
          {/* ── Première visite ── */}
          <p className="text-xs font-semibold text-kleia-violet uppercase tracking-wider mb-4">Première visite</p>
          <GoogleButton />
          <p className="mt-4 text-center">
            <Link to="/register"
              data-testid="login-register-link"
              className="inline-flex items-center justify-center w-full py-2.5 px-4 rounded-lg border-2 border-kleia-violet/20 text-kleia-violet font-semibold text-sm font-heading hover:bg-kleia-violet/5 transition-colors">
