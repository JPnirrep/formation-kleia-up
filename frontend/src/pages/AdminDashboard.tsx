...
export default function AdminDashboard() {
  ...
  const actions = [
    { label: 'Créer une formation', icon: '📝', action: () => navigate('/admin/courses/new'), variant: 'primary' as const },
    { label: 'Gérer les formations', icon: '📚', action: () => navigate('/admin/courses'), variant: 'outline' as const },
    { label: 'Gérer les utilisateurs', icon: '👥', action: () => navigate('/admin/users'), variant: 'outline' as const },
  ];
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recentEnrollments.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-kleia-gray font-body">Aucune donnée</td>
                  </tr>
                ) : (
